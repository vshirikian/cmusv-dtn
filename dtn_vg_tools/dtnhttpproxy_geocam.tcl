...
proc usage {} {
    ...
    puts "    -u | --url <url>   Set url (default is http://localhost:8888/upload/"
}
...
proc proxy_loop {} {
	...
	# vache - hardcode geocam share web server ip for now
	...
	if {$url == ""} {
        ...
        switch $params(mode) {
            upload {
                ...
                set fd [open $payload_file]
		# vache - edit for geocam boundry
                #set params(content_type) "multipart/form-data, boundary=xYzZY"
                #set params(content_type) "multipart/form-data, boundary=--------multipart_formdata_boundary$--------" 
                set params(content_type) "multipart/form-data"
