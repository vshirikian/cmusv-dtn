...
import se.kth.ssvl.tslab.bytewalla.androiddtn.servlib.contacts.LinkStats;
...
public class Link implements Serializable {
	...
	public String state_to_str(state_t state) {
		switch (state) {
		case UNAVAILABLE:
			return "UNAVAILABLE";
		case AVAILABLE:
	...
	public Link(String name, link_type_t type, ConvergenceLayer cl,
		...
		try {
			dest_ip_ = InetAddress.getByName(parameters[0]);
		} catch (IOException e) {
