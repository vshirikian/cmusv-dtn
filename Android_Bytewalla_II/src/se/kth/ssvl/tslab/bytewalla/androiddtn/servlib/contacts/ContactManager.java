...
public class ContactManager extends BundleEventHandler {
	...
	public boolean add_new_link(Link link) {
		...
		try {
			...
			Log.d(TAG, "adding NEW link %s" + link.name());
			if (has_link(link)) {
				return false;
			}
			links_.add(link);
	 ...
	 */
	public Link find_link_to(ConvergenceLayer cl, String nexthop,
			EndpointID remote_eid, Link.link_type_t type, int states) {
		...
		try {
			...
			String text = String
					.format(
							"find_link_to: cl %s nexthop %s remote_eid %s type %s states 0x%x",
							cl.name(), nexthop, remote_eid, Link
									.link_type_to_str(type), states);

			Log.d(TAG, text);

			// make sure some sane criteria was specified
			assert ((cl != null) || (nexthop != "")
					|| (remote_eid != EndpointID.NULL_EID()) || (type != Link.link_type_t.LINK_INVALID)) : "ContactManager : Link_find_to, invalid parameters";

			while (iter.hasNext()) {

				Link element = iter.next();

				if (((type == Link.link_type_t.LINK_INVALID) || (type == element
						.type()))
						&& ((cl == null) || (element.clayer() == cl))
						&& ((nexthop == "") || (element.nexthop() == nexthop))
						&& (remote_eid.equals(EndpointID.NULL_EID()))
						|| (remote_eid.equals(element.remote_eid()))
						&& ((states & element.state().getCode()) > 0)) {
	...
	public Link new_opportunistic_link(ConvergenceLayer cl, String nexthop,
			EndpointID remote_eid, String link_name) {
