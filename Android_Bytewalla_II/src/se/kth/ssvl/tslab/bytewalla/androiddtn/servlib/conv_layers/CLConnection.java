...
import se.kth.ssvl.tslab.bytewalla.androiddtn.servlib.conv_layers.LinkParams;
...
public abstract class CLConnection extends CLInfo implements Runnable {
	...
	boolean find_contact(EndpointID peer_eid) {
		...
		Link link = cm.find_link_to(cl_, "", peer_eid,
				Link.link_type_t.OPPORTUNISTIC, Link.state_t.AVAILABLE
						.getCode()
						| Link.state_t.UNAVAILABLE.getCode());
		...
		if (link == null || (link != null && link.contact() != null)) {
			...
			link = cm.new_opportunistic_link(cl_, nexthop_, peer_eid, null);
			if (link == null) {
