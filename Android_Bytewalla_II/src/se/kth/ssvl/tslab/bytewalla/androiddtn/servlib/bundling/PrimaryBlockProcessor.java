...
public class PrimaryBlockProcessor extends BlockProcessor implements Serializable{
     ...
     * @param block (OUT): Primary block to set data after consuming
     * @param buffer (IN): Populated buffer to read data from for consuming 
    ...
    public int consume(Bundle bundle,
    	...
    	//temp primary block where we keep the parsed values, prior to save them into bundle.
...
//      	byte_buffer_temp = BufferHelper.reserve(byte_buffer_temp, len);
        ...
        Log.e(TAG, " primary_len: "+primary_len+" : len:"+len);
...
//        dictionary.position((int)(primary_len-primary.dictionary_length_value()));
     ...
     * @param xmit_blocks (): Empty xmit_blocks of the bundle
     ...
     * @param bundle (OUT): Bundle to generate
...
//        buf += dict->length();
    ...
    /**
     * decode the value from the buffer and return it. 
     ...
     * @param bundle (OUT): Bundle to set the flag values
     * @param flags (IN): Formatted value of flags
     */
