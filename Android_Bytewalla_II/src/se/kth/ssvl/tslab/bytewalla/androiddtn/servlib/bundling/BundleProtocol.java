...
public class BundleProtocol {
	 ...
	 * xmit_blocks_." [DTN2]. Updated for security functionality.
	 * We create a BlockInfoVec with the needed blockInfo elements (which are empty), 
	 * and we set it to the bundle.xmit_link_block_. 
	 * @param bundle ():
	 * @param link (IN):
	...
	public static BlockInfoVec prepare_blocks(Bundle bundle, final Link link) 
	{
	    ...
	    if (recv_blocks.size() > 0) //when is it executed?
	    {
	        ...
	        while(iter.hasNext())
	        {
	            ...
	            if (bundle.fragmented_incoming() && xmit_blocks.find_block(BundleProtocol.bundle_block_type_t.PAYLOAD_BLOCK) != null) 
	            {
	                continue;
	            }
	            
	            block.owner().prepare(bundle, xmit_blocks, block, link, BlockInfo.list_owner_t.LIST_RECEIVED);
	        }
	    }
	    else  //recv_blocks.size() = 0 when we are sending a bundle from scratch.
	    {
	        ...
	        bp.prepare(bundle, xmit_blocks, null, link, BlockInfo.list_owner_t.LIST_NONE);//we call primaryBlockProcessor.prepare
	        bp = find_processor(bundle_block_type_t.PAYLOAD_BLOCK);
	        bp.prepare(bundle, xmit_blocks, null, link, BlockInfo.list_owner_t.LIST_NONE);//we call BlockProcessor.prepare (payloadBlockProcessor doesn't implement "prepare").
	    }
	...
	/**
	 * Prepare_blocks has already created the empty structure of blocks into BlockInfoVec. Now, generate_blocks
	 * Generates contents for the given BlockInfoVec on the given Link." 
	 * 
			...
			final Link link) 
	{
	    ...
	    // to the beginning, in order for security processing to work.
	 ...
	 * primary block." [DTN2]
	 * XXX sd. defined in page 15 of RFC5050.
	 */
