...
import se.kth.ssvl.tslab.bytewalla.androiddtn.servlib.discovery.Discovery;
...
public class IPDiscovery extends Discovery implements Runnable {
	...
	private static WifiManager mWifi = (WifiManager) DTNService.context()
			.getSystemService(DTNService.WIFI_SERVICE);
	...
	/**
	 * Get the current ip addres of the mobile. Get the ip assigned by the DHCP
	...
	protected boolean configure() {
		...
		try {
			socket_ = new DatagramSocket(port_, local_addr_);
	...
	public void run() {
		...
		socket_.connect(remote_addr_, 3000);
		...
		while (true) {
			...
			while (i.hasNext()) {
				...
				if (socket_.getLocalAddress().equals(announce.cl_addr())) {
					...
					if (remaining == 0) {
						...
						byte[] ip_address = hdr.inet_addr().getAddress();
						buf.put(ip_address);
					...
					Log
							.d(TAG,
									"Local address is diferent to the announce cl address");
					return;
				}
			...
			try {
				...
				if (remote_eid.equals(BD.local_eid())) {
					Log.d(TAG, "ignoring beacon from self" + remote_addr_);
				...
				Log.d(TAG, "Fail receiving the UDP datagram " + e.getMessage());
			}
	...
	protected InetAddress BROADCAST; // / Broadcast address of the network where the phone is connected
